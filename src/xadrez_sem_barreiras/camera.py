...
class BoardDetector:
    def __init__(self, camera_index=0):
        ...
        # IMPORTANTE:
        # A imagem gerada pelo OpenCV tem pixels de 0 até tamanho_tabuleiro - 1.
        # Usar o ponto 800 em uma imagem 800x800 joga a borda para fora da imagem
        # e pode criar um pequeno desalinhamento da grade verde.
        ultimo_pixel = self.tamanho_tabuleiro - 1
    ...
    def desenhar_grade_para_teste(self, tabuleiro_plano):
        ...
        A grade usa as dimensões reais da imagem recebida. Isso evita erro quando a
        imagem é redimensionada ou quando o tabuleiro foi retificado com margem.
        """
        imagem_teste = tabuleiro_plano.copy()
        altura, largura = imagem_teste.shape[:2]

        for i in range(1, 8):
            x = round(i * largura / 8)
            y = round(i * altura / 8)
            cv2.line(imagem_teste, (0, y), (largura - 1, y), (0, 255, 0), 2)
            cv2.line(imagem_teste, (x, 0), (x, altura - 1), (0, 255, 0), 2)
